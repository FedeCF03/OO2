...
public abstract class SingleThreadTCPServer {
    ...
    private final void handleClient(Socket clientSocket) {
        ...
        try (
            ...
            while ((inputLine = in.readLine()) != null) {
                ...
                if (cerrarConexionCondicion(inputLine)) {
                    break; // Client requested to close the connection
                }
                handleMessage(inputLine, out);
            }
            postConexion(clientSocket, out);
        } catch (IOException e) {
            handleMensajeError(e);
        } finally {
    ...
    protected boolean cerrarConexionCondicion(String inputLine) {
        return inputLine.equalsIgnoreCase("");
    }

    protected void postConexion(Socket clientSocket, PrintWriter out) {
    };

    protected void handleMensajeError(IOException e) {
